...
build() {
  ...
  ROOT_CA=${OUTPATH}rootCA.pem
  ROOT_KEY=${OUTPATH}rootCA.key

  if [ ! -f ${ROOT_CA} ]; then

    # Generate CA key & crt
    ROOT_CA=${OUTPATH}${FILENAME}_CA.pem
    ROOT_KEY=${OUTPATH}${FILENAME}_CA.key

    openssl genrsa -out ${ROOT_KEY} 2048
    openssl req -x509 -new -nodes -key ${ROOT_KEY} -sha256 -days ${DURATION} -out ${ROOT_CA} -subj "/C=${C}/ST=${ST}/L=${L}/O=${O}/OU=${OU}/CN=${CN}/emailAddress=${emailAddress}"
  fi
  ...
  # Server certificate
  openssl x509 -req -in ${OUTPATH}${FILENAME}.csr -CA ${ROOT_CA} -CAkey ${ROOT_KEY} -CAcreateserial -out ${OUTPATH}${FILENAME}.crt -days ${DURATION} -sha256 -extfile ${tmp}/v3.ext
}
